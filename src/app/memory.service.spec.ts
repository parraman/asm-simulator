...
import {MemoryOperation, MemoryService, CellType} from './memory.service';
...
describe('MemoryService', () => {
  ...
  it('add memory region', inject([MemoryService], (service: MemoryService) => {
    ...
    const createRegion = function(startAddress: number, endAddress: number) {
      service.addMemoryRegion(startAddress, endAddress, CellType.READ_WRITE, emitter);
    };
    let region = service.addMemoryRegion(10, 800, CellType.READ_WRITE, emitter);
    ...
    region = service.addMemoryRegion(801, 1023, CellType.READ_WRITE, emitter);
    expect(region).toBeTruthy();
